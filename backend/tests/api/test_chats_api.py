"""Comprehensive tests for Chats API endpoints."""

import pytest
from uuid import uuid4
from unittest.mock import AsyncMock, patch

from app.models.chat import Chat
from app.models.message import Message
from app.models.user import User


@pytest.fixture
async def test_user(db):
    """Create a test user."""
    user = User(
        id=uuid4(),
        email="chatuser@test.com",
        full_name="Chat User",
        google_id="google_test_id",
        is_active=True,
    )
    db.add(user)
    await db.commit()
    await db.refresh(user)
    return user


@pytest.fixture
async def test_chat(db, test_user):
    """Create a test chat."""
    chat = Chat(
        title="Test Chat",
        user_id=test_user.id,
    )
    db.add(chat)
    await db.commit()
    await db.refresh(chat)
    return chat


@pytest.fixture
async def test_chat_with_messages(db, test_user):
    """Create a test chat with messages."""
    chat = Chat(
        title="Chat with Messages",
        user_id=test_user.id,
    )
    db.add(chat)
    await db.commit()
    await db.refresh(chat)
    
    # Add some messages
    messages = [
        Message(
            chat_id=chat.id,
            role="user",
            content="Hello, how are you?",
        ),
        Message(
            chat_id=chat.id,
            role="assistant",
            content="I'm doing well, thank you!",
        ),
        Message(
            chat_id=chat.id,
            role="user",
            content="Can you help me with something?",
        ),
    ]
    
    for msg in messages:
        db.add(msg)
    
    await db.commit()
    
    # Refresh to load messages
    await db.refresh(chat)
    
    return chat


@pytest.fixture
def mock_auth_user(test_user):
    """Mock authentication dependency."""
    async def _get_current_user():
        return test_user
    return _get_current_user


class TestCreateChat:
    """Tests for POST /api/v1/chats endpoint."""

    @pytest.mark.asyncio
    async def test_create_chat_success(self, async_client, test_user, mock_auth_user):
        """Test successful chat creation."""
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await async_client.post(
                "/api/v1/chats",
                json={"title": "My New Chat"},
            )
        
        assert response.status_code == 201
        data = response.json()
        assert data["title"] == "My New Chat"
        assert data["user_id"] == str(test_user.id)
        assert "id" in data
        assert "created_at" in data
        assert "updated_at" in data

    @pytest.mark.asyncio
    async def test_create_chat_with_empty_title(self, async_client, test_user, mock_auth_user):
        """Test chat creation with empty title."""
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.post(
                "/api/v1/chats",
                json={"title": ""},
            )
        
        assert response.status_code == 201
        data = response.json()
        assert data["title"] == ""

    @pytest.mark.asyncio
    async def test_create_chat_unauthorized(self, client):
        """Test chat creation without authentication."""
        response = await client.post(
            "/api/v1/chats",
            json={"title": "Unauthorized Chat"},
        )
        
        assert response.status_code == 401


class TestListChats:
    """Tests for GET /api/v1/chats endpoint."""

    @pytest.mark.asyncio
    async def test_list_chats_empty(self, async_client, test_user, mock_auth_user):
        """Test listing chats when user has none."""
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.get("/api/v1/chats")
        
        assert response.status_code == 200
        data = response.json()
        assert data["chats"] == []
        assert data["total"] == 0

    @pytest.mark.asyncio
    async def test_list_chats_with_data(self, async_client, test_user, test_chat, mock_auth_user, db):
        """Test listing chats with existing data."""
        # Create additional chats
        chats = [
            Chat(title=f"Chat {i}", user_id=test_user.id)
            for i in range(3)
        ]
        for chat in chats:
            db.add(chat)
        await db.commit()
        
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.get("/api/v1/chats")
        
        assert response.status_code == 200
        data = response.json()
        assert len(data["chats"]) == 4  # test_chat + 3 new chats
        assert data["total"] == 4

    @pytest.mark.asyncio
    async def test_list_chats_pagination(self, async_client, test_user, mock_auth_user, db):
        """Test chat list pagination."""
        # Create 10 chats
        chats = [
            Chat(title=f"Chat {i}", user_id=test_user.id)
            for i in range(10)
        ]
        for chat in chats:
            db.add(chat)
        await db.commit()
        
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            # Get first page
            response = await client.get("/api/v1/chats?skip=0&limit=5")
            assert response.status_code == 200
            data = response.json()
            assert len(data["chats"]) == 5
            assert data["total"] == 10
            
            # Get second page
            response = await client.get("/api/v1/chats?skip=5&limit=5")
            assert response.status_code == 200
            data = response.json()
            assert len(data["chats"]) == 5
            assert data["total"] == 10

    @pytest.mark.asyncio
    async def test_list_chats_user_isolation(self, async_client, test_user, test_chat, mock_auth_user, db):
        """Test that users only see their own chats."""
        # Create another user and their chat
        other_user = User(
            id=uuid4(),
            email="other@test.com",
            google_id="google_other_id",
            full_name="Other User",
        )
        db.add(other_user)
        await db.commit()
        
        other_chat = Chat(
            title="Other User's Chat",
            user_id=other_user.id,
        )
        db.add(other_chat)
        await db.commit()
        
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.get("/api/v1/chats")
        
        assert response.status_code == 200
        data = response.json()
        # Should only see test_chat, not other_chat
        assert data["total"] == 1
        assert data["chats"][0]["title"] == "Test Chat"

    @pytest.mark.asyncio
    async def test_list_chats_unauthorized(self, client):
        """Test listing chats without authentication."""
        response = await client.get("/api/v1/chats")
        assert response.status_code == 401


class TestGetChat:
    """Tests for GET /api/v1/chats/{chat_id} endpoint."""

    @pytest.mark.asyncio
    async def test_get_chat_success(self, async_client, test_user, test_chat, mock_auth_user):
        """Test successful chat retrieval."""
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.get(f"/api/v1/chats/{test_chat.id}")
        
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == str(test_chat.id)
        assert data["title"] == test_chat.title
        assert "messages" in data

    @pytest.mark.asyncio
    async def test_get_chat_with_messages(self, async_client, test_user, test_chat_with_messages, mock_auth_user):
        """Test retrieving chat with messages."""
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.get(f"/api/v1/chats/{test_chat_with_messages.id}")
        
        assert response.status_code == 200
        data = response.json()
        assert len(data["messages"]) == 3
        assert data["messages"][0]["role"] == "user"
        assert data["messages"][0]["content"] == "Hello, how are you?"
        assert data["messages"][1]["role"] == "assistant"

    @pytest.mark.asyncio
    async def test_get_chat_not_found(self, async_client, test_user, mock_auth_user):
        """Test retrieving non-existent chat."""
        fake_id = uuid4()
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.get(f"/api/v1/chats/{fake_id}")
        
        assert response.status_code == 404
        assert "not found" in response.json()["detail"].lower()

    @pytest.mark.asyncio
    async def test_get_chat_wrong_user(self, async_client, test_user, test_chat, mock_auth_user, db):
        """Test retrieving another user's chat."""
        # Create another user
        other_user = User(
            id=uuid4(),
            email="hacker@test.com",
            google_id="google_other_id",
            full_name="Hacker User",
        )
        db.add(other_user)
        await db.commit()
        
        # Mock as other user
        async def _get_other_user():
            return other_user
        
        with patch("app.api.dependencies.get_current_user", _get_other_user):
            response = await client.get(f"/api/v1/chats/{test_chat.id}")
        
        # Should return 404 (not 403) to avoid leaking existence
        assert response.status_code == 404

    @pytest.mark.asyncio
    async def test_get_chat_invalid_uuid(self, async_client, test_user, mock_auth_user):
        """Test retrieving chat with invalid UUID."""
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.get("/api/v1/chats/not-a-uuid")
        
        assert response.status_code == 422

    @pytest.mark.asyncio
    async def test_get_chat_unauthorized(self, async_client, test_chat):
        """Test retrieving chat without authentication."""
        response = await client.get(f"/api/v1/chats/{test_chat.id}")
        assert response.status_code == 401


class TestUpdateChat:
    """Tests for PATCH /api/v1/chats/{chat_id} endpoint."""

    @pytest.mark.asyncio
    async def test_update_chat_title(self, async_client, test_user, test_chat, mock_auth_user):
        """Test updating chat title."""
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.patch(
                f"/api/v1/chats/{test_chat.id}",
                json={"title": "Updated Title"},
            )
        
        assert response.status_code == 200
        data = response.json()
        assert data["title"] == "Updated Title"

    @pytest.mark.asyncio
    async def test_update_chat_empty_title(self, async_client, test_user, test_chat, mock_auth_user):
        """Test updating chat with empty title."""
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.patch(
                f"/api/v1/chats/{test_chat.id}",
                json={"title": ""},
            )
        
        assert response.status_code == 200
        data = response.json()
        assert data["title"] == ""

    @pytest.mark.asyncio
    async def test_update_chat_no_changes(self, async_client, test_user, test_chat, mock_auth_user):
        """Test updating chat with no changes."""
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.patch(
                f"/api/v1/chats/{test_chat.id}",
                json={},
            )
        
        assert response.status_code == 200
        data = response.json()
        assert data["title"] == test_chat.title

    @pytest.mark.asyncio
    async def test_update_chat_not_found(self, async_client, test_user, mock_auth_user):
        """Test updating non-existent chat."""
        fake_id = uuid4()
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.patch(
                f"/api/v1/chats/{fake_id}",
                json={"title": "New Title"},
            )
        
        assert response.status_code == 404

    @pytest.mark.asyncio
    async def test_update_chat_wrong_user(self, async_client, test_user, test_chat, mock_auth_user, db):
        """Test updating another user's chat."""
        # Create another user
        other_user = User(
            id=uuid4(),
            email="attacker@test.com",
            google_id="google_other_id",
            full_name="Attacker",
        )
        db.add(other_user)
        await db.commit()
        
        # Mock as other user
        async def _get_other_user():
            return other_user
        
        with patch("app.api.dependencies.get_current_user", _get_other_user):
            response = await client.patch(
                f"/api/v1/chats/{test_chat.id}",
                json={"title": "Hacked Title"},
            )
        
        assert response.status_code == 404

    @pytest.mark.asyncio
    async def test_update_chat_unauthorized(self, async_client, test_chat):
        """Test updating chat without authentication."""
        response = await client.patch(
            f"/api/v1/chats/{test_chat.id}",
            json={"title": "No Auth"},
        )
        assert response.status_code == 401


class TestDeleteChat:
    """Tests for DELETE /api/v1/chats/{chat_id} endpoint."""

    @pytest.mark.asyncio
    async def test_delete_chat_success(self, async_client, test_user, test_chat, mock_auth_user, db):
        """Test successful chat deletion."""
        chat_id = test_chat.id
        
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.delete(f"/api/v1/chats/{chat_id}")
        
        assert response.status_code == 204
        
        # Verify chat is deleted
        from sqlalchemy import select
        from app.models.chat import Chat
        
        result = await db.execute(select(Chat).where(Chat.id == chat_id))
        deleted_chat = result.scalar_one_or_none()
        assert deleted_chat is None

    @pytest.mark.asyncio
    async def test_delete_chat_not_found(self, async_client, test_user, mock_auth_user):
        """Test deleting non-existent chat."""
        fake_id = uuid4()
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.delete(f"/api/v1/chats/{fake_id}")
        
        assert response.status_code == 404

    @pytest.mark.asyncio
    async def test_delete_chat_wrong_user(self, async_client, test_user, test_chat, mock_auth_user, db):
        """Test deleting another user's chat."""
        # Create another user
        other_user = User(
            id=uuid4(),
            email="badactor@test.com",
            google_id="google_other_id",
            full_name="Bad Actor",
        )
        db.add(other_user)
        await db.commit()
        
        # Mock as other user
        async def _get_other_user():
            return other_user
        
        with patch("app.api.dependencies.get_current_user", _get_other_user):
            response = await client.delete(f"/api/v1/chats/{test_chat.id}")
        
        assert response.status_code == 404
        
        # Verify chat still exists
        from sqlalchemy import select
        from app.models.chat import Chat
        
        result = await db.execute(select(Chat).where(Chat.id == test_chat.id))
        existing_chat = result.scalar_one_or_none()
        assert existing_chat is not None

    @pytest.mark.asyncio
    async def test_delete_chat_with_messages(self, async_client, test_user, test_chat_with_messages, mock_auth_user, db):
        """Test deleting chat with messages (cascade)."""
        chat_id = test_chat_with_messages.id
        
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.delete(f"/api/v1/chats/{chat_id}")
        
        assert response.status_code == 204
        
        # Verify chat and messages are deleted
        from sqlalchemy import select
        from app.models.chat import Chat
        from app.models.message import Message
        
        result = await db.execute(select(Chat).where(Chat.id == chat_id))
        assert result.scalar_one_or_none() is None
        
        result = await db.execute(select(Message).where(Message.chat_id == chat_id))
        messages = result.scalars().all()
        assert len(messages) == 0  # Should be cascade deleted

    @pytest.mark.asyncio
    async def test_delete_chat_unauthorized(self, async_client, test_chat):
        """Test deleting chat without authentication."""
        response = await client.delete(f"/api/v1/chats/{test_chat.id}")
        assert response.status_code == 401


class TestChatEdgeCases:
    """Edge case and integration tests."""

    @pytest.mark.asyncio
    async def test_chat_ordering(self, async_client, test_user, mock_auth_user, db):
        """Test that chats are ordered by updated_at DESC."""
        # Create chats in sequence
        import asyncio
        chats = []
        for i in range(3):
            chat = Chat(title=f"Chat {i}", user_id=test_user.id)
            db.add(chat)
            await db.commit()
            await db.refresh(chat)
            chats.append(chat)
            await asyncio.sleep(0.01)  # Ensure different timestamps
        
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.get("/api/v1/chats")
        
        assert response.status_code == 200
        data = response.json()
        
        # Should be reverse order (most recent first)
        titles = [c["title"] for c in data["chats"]]
        assert titles[0] == "Chat 2"
        assert titles[-1] == "Chat 0"

    @pytest.mark.asyncio
    async def test_chat_long_title(self, async_client, test_user, mock_auth_user):
        """Test creating chat with very long title."""
        long_title = "A" * 10000
        
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.post(
                "/api/v1/chats",
                json={"title": long_title},
            )
        
        assert response.status_code == 201
        data = response.json()
        assert len(data["title"]) == 10000

    @pytest.mark.asyncio
    async def test_chat_special_characters(self, async_client, test_user, mock_auth_user):
        """Test creating chat with special characters in title."""
        special_title = "🚀 Test Chat with émojis & spëcial chars: <script>alert('xss')</script>"
        
        with patch("app.api.dependencies.get_current_user", mock_auth_user):
            response = await client.post(
                "/api/v1/chats",
                json={"title": special_title},
            )
        
        assert response.status_code == 201
        data = response.json()
        assert data["title"] == special_title
