...
class TestTemplateServiceUseTemplate:
    ...
    async def test_use_template_supports_percent_and_pct_transforms(
        ...
        """percent/pct should format ratio values as readable percentages."""
        service, db = service_with_mock_db
        template_id = uuid4()
        user_id = uuid4()
        template = SimpleNamespace(
            id=template_id,
            prompt_template=(
                "Conversion: {ratio->percent}, "
                "Precise: {ratio->percent(2)}, "
                "Alias: {ratio->pct(1)}"
            ),
            category="docs",
            usage_count=0,
        ...
        assert result["prompt"] == "Conversion: 13%, Precise: 12.60%, Alias: 12.6%"
        assert template.usage_count == 1
        db.commit.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_use_template_rejects_percent_transform_for_non_numeric_values(
        self, service_with_mock_db
    ):
        """percent should fail fast for non-numeric values."""
        service, db = service_with_mock_db
        template_id = uuid4()
        user_id = uuid4()
        template = SimpleNamespace(
            id=template_id,
            prompt_template="Conversion: {ratio->percent}",
            category="docs",
            usage_count=2,
        )
        ...
        with patch.object(service, "get_template", AsyncMock(return_value=template)):
            with pytest.raises(
                ValueError,
                match=r"Failed to apply template transform 'percent'",
            ):
                await service.use_template(
                    template_id,
                    {"ratio": "n/a"},
    ...
    @pytest.mark.asyncio
    async def test_use_template_rejects_percent_transform_with_invalid_precision(
        self, service_with_mock_db
    ):
        """percent(ndigits) should require a non-negative integer precision."""
        service, db = service_with_mock_db
        template_id = uuid4()
        user_id = uuid4()
        template = SimpleNamespace(
            id=template_id,
            prompt_template="Conversion: {ratio->percent(-1)}",
            category="docs",
            usage_count=1,
        )
        ...
        with patch.object(service, "get_template", AsyncMock(return_value=template)):
            with pytest.raises(
                ValueError,
                match=r"Failed to apply template transform 'percent\(-1\)'",
            ):
                await service.use_template(
                    template_id,
                    {"ratio": 0.42},
    ...
    async def test_use_template_supports_geometric_mean_numeric_transforms(
        ...
        """geomean/gmean should return geometric means for numeric iterables."""
        service, db = service_with_mock_db
        template_id = uuid4()
        user_id = uuid4()
        template = SimpleNamespace(
        ...
        assert result["prompt"] == "Geo mean: 3, Alias: 3"
        assert template.usage_count == 3
        db.commit.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_use_template_supports_geometric_mean_with_zero_values(
        self, service_with_mock_db
    ):
        """geomean should return zero when any iterable value is zero."""
        service, db = service_with_mock_db
        template_id = uuid4()
        user_id = uuid4()
        template = SimpleNamespace(
            id=template_id,
            prompt_template="Geo mean: {durations->geomean}",
            category="docs",
            usage_count=0,
        )
        ...
        assert result["prompt"] == "Geo mean: 0"
        assert template.usage_count == 1
        db.commit.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_use_template_rejects_geometric_mean_transform_for_negative_values(
        self, service_with_mock_db
    ):
        """geomean should reject negative values to avoid complex-number output."""
        service, db = service_with_mock_db
        template_id = uuid4()
        user_id = uuid4()
        template = SimpleNamespace(
        ...
        with patch.object(service, "get_template", AsyncMock(return_value=template)):
            with pytest.raises(
                ValueError,
                match=r"Failed to apply template transform 'geomean'",
            ):
                await service.use_template(
                    template_id,
                    {"durations": [-1, 3, 9]},
    ...
    @pytest.mark.asyncio
    async def test_use_template_supports_harmonic_mean_numeric_transforms(
        self, service_with_mock_db
    ):
        """harmmean/hmean/harmonic_mean should return harmonic means for iterables."""
        service, db = service_with_mock_db
        template_id = uuid4()
        user_id = uuid4()
        template = SimpleNamespace(
            id=template_id,
            prompt_template=(
                "Harm mean: {durations->harmmean->round(3)}, "
                "Alias: {durations->hmean()->round(3)}, "
                "Function: {durations->harmonic_mean()->round(3)}"
            ),
            category="docs",
            usage_count=1,
        )
        ...
        assert result["prompt"] == "Harm mean: 1.714, Alias: 1.714, Function: 1.714"
        assert template.usage_count == 2
        db.commit.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_use_template_rejects_harmonic_mean_transform_for_non_positive_values(
        self, service_with_mock_db
    ):
        """harmmean should reject zero and negative values."""
        service, db = service_with_mock_db
        template_id = uuid4()
        user_id = uuid4()
        template = SimpleNamespace(
            id=template_id,
            prompt_template="Harm mean: {durations->harmmean}",
            category="docs",
            usage_count=3,
        )
        ...
        with patch.object(service, "get_template", AsyncMock(return_value=template)):
            with pytest.raises(
                ValueError,
                match=r"Failed to apply template transform 'harmmean'",
            ):
                await service.use_template(
                    template_id,
                    {"durations": [0, 2, -4]},
    ...
    @pytest.mark.asyncio
    async def test_use_template_rejects_harmonic_mean_transform_with_arguments(
        self, service_with_mock_db
    ):
        """harmmean should reject arguments to keep semantics deterministic."""
        service, db = service_with_mock_db
        template_id = uuid4()
        user_id = uuid4()
        template = SimpleNamespace(
            id=template_id,
            prompt_template="Harm mean: {durations->harmmean(weighted)}",
            category="docs",
            usage_count=2,
        )
        ...
        with patch.object(service, "get_template", AsyncMock(return_value=template)):
            with pytest.raises(
                ValueError,
                match=r"Failed to apply template transform 'harmmean\(weighted\)'",
            ):
                await service.use_template(
                    template_id,
                    {"durations": [1, 2, 4]},
    ...
    async def test_use_template_supports_min_and_max_numeric_transforms(
        ...
        """min/max should return numeric bounds for iterable metrics."""
        service, db = service_with_mock_db
        template_id = uuid4()
        user_id = uuid4()
        template = SimpleNamespace(
            id=template_id,
            prompt_template="Min latency: {latencies->min}, Max latency: {latencies->max}",
            category="docs",
            usage_count=0,
        )
        ...
        assert result["prompt"] == "Min latency: 3.5, Max latency: 12"
        assert template.usage_count == 1
        db.commit.assert_awaited_once()
    ...
    async def test_use_template_rejects_min_transform_for_empty_collections(
        ...
        """min should fail for empty iterables to avoid ambiguous defaults."""
        service, db = service_with_mock_db
        template_id = uuid4()
        user_id = uuid4()
        template = SimpleNamespace(
            id=template_id,
            prompt_template="Min latency: {latencies->min}",
            category="docs",
            usage_count=2,
        )
        ...
        with patch.object(service, "get_template", AsyncMock(return_value=template)):
            with pytest.raises(
                ValueError,
                match=r"Failed to apply template transform 'min'",
            ):
                await service.use_template(
                    template_id,
                    {"latencies": []},
    ...
    @pytest.mark.asyncio
    async def test_use_template_rejects_max_transform_for_non_numeric_values(
        self, service_with_mock_db
    ):
        """max should fail fast when iterable members are not numeric."""
        service, db = service_with_mock_db
        template_id = uuid4()
        user_id = uuid4()
        template = SimpleNamespace(
            id=template_id,
            prompt_template="Max latency: {latencies->max}",
            category="docs",
            usage_count=3,
        )
        ...
        with patch.object(service, "get_template", AsyncMock(return_value=template)):
            with pytest.raises(
                ValueError,
                match=r"Failed to apply template transform 'max'",
            ):
                await service.use_template(
                    template_id,
                    {"latencies": [5, "n/a", 3]},
